...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//BYTE read_data;
	int ret_code;
	ret_code = alt_avalon_spi_command(SPI_0_BASE, 0, 2, &w, 0, NULL, 0);
	if(ret_code < 0)
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int ret_code;
	//BYTE read_data;
	ret_code = alt_avalon_spi_command(SPI_0_BASE, 0, nbytes + 1, &w, 0, NULL, 0);
	if(ret_code < 0)
		printf("Error! Return code less than 0.");
	return (data + nbytes);
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	BYTE val;
	int ret_code;
	ret_code = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, 1, &val, 0);
	if(ret_code < 0)
			printf("Error! Return code less than 0.");
	return val;
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//BYTE w[1] = reg;
	int ret_code;
	ret_code = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, nbytes, data, 0);
	if(ret_code < 0)
			printf("Error! Return code less than 0.");
	return (data + nbytes);
}
